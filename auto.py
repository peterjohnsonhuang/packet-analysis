...
def get_http_headers(http_payload,host_list):
...
def http_assembler_auto(pcap_file,host_list):
	...
	for session in sessions:
		...
		for payload in payload_list:
			headers = get_http_headers(payload,host_list)
			...
			if data is not None and data_type is not None and data not in data_list:				
				...
				# store the data
				if (data_category not in captured_data):
					captured_data[data_category] = {};
					captured_data[data_category][data_type] = 1			
				else:
					if (data_type not in captured_data[data_category]):
						captured_data[data_category][data_type] = 1
					else:
						captured_data[data_category][data_type] += 1 
				file_name = "%s-%s%d.%s" % (pcap_file.split(".")[0],data_type,captured_data[data_category][data_type],data_type)
